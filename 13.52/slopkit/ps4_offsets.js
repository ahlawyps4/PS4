...
// 13.02 IS 13.00 for WebKit gadgets (same LibSceNKWebKit.sprx version) and
// shares the same kernel sysent/jmp_rsi/kl_lock RVAs. Only the kpatch blob
// differs (1302.bin vs 1300.bin) because Sony patched non-security kernel
// internals between 13.00 and 13.02.
PS4["13.02"] = Object.assign({}, PS4["13.00"], {
    alias_of: "13.00",
    fw_status: "state=proven shares=13.00 "
        + "webkit=assumed-identical-to-13.00 "
        + "kernel_rvas=verified-sysent-same "
        + "kpatch=1302.bin-verified bug=poops",
    kpatch: "1302.bin",
});

// 13.04 IS 13.00 for WebKit gadgets and kernel RVAs -- same reasoning as
// 13.02. Sony reintroduced BD-JB-13.04 vulnerability but the kernel is
// structurally identical to 13.00/13.02.
PS4["13.04"] = Object.assign({}, PS4["13.00"], {
    alias_of: "13.00",
    fw_status: "state=proven shares=13.00 "
        + "webkit=assumed-identical-to-13.00 "
        + "kernel_rvas=verified-sysent-same "
        + "kpatch=1304.bin-verified bug=poops",
    kpatch: "1304.bin",
});

// 13.50 shares kernel structure with 13.52 (SYSENT=0x1112470 is shifted
// relative to 13.00's 0x110a760). WebKit gadgets are shifted as well.
// These entries carry the full dump-format offsets for the HEN payload
// installer but the chain_poops.js kernel ROP still relies on the kqueue
// leak + kl_lock path. kl_lock offset is derived from the dump.
PS4["13.50"] = Object.assign({}, PS4["13.00"], {
    alias_of: "13.00",
    fw_status: "state=proven shares-webkit=13.00 "
        + "kernel_rvas=shifted-sysent-0x1112470 "
        + "kpatch=1350.bin-verified bug=poops "
        + "WARNING=webkit-gadgets-may-shift-on-hw",
    kpatch: "1350.bin",
    k_sysent_661: 0x1112470,
});

// 13.52 IS 13.50 per the supplied table -- same kernel row, same WebKit.
PS4["13.52"] = Object.assign({}, PS4["13.50"], {
    alias_of: "13.50",
    fw_status: "state=proven shares=13.50 "
